...
def search_image(model_manager, image_input, threshold, model_name, filter_options=None, multiband_data=None):
    ...
    try:
        ...
        results = _fetch_top_k_images(top_indices, probs, df_embed, model_manager=model_manager, query_text="Image Query")
        timings["Download"] = time.time() - t0
...
def search_location(model_manager, lat, lon, threshold, filter_options=None):
    ...
    try:
        ...
        results = _fetch_top_k_images(top_6_indices, probs, df_embed, model_manager=model_manager, query_text=f"Loc: {lat},{lon}")
...
def search_mixed(
    ...
    try:
        ...
        results = _fetch_top_k_images(top_indices, final_scores, df_ref, model_manager=model_manager, query_text=query_info)
        timings["Download"] = time.time() - t0
...
def _fetch_top_k_images(top_indices, probs, df_embed, model_manager=None, query_text=None):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for _i, idx in enumerate(top_indices):
            ...
            future = executor.submit(download_and_process_image, pid, df_source=df_source, verbose=False, mode="thumbnail")
