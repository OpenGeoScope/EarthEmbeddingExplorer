...
from open_clip.model import get_cast_dtype, trace_model
from open_clip.factory import create_model_and_transforms
from open_clip_train.logger import setup_logging
from open_clip_train.distributed import is_master, init_distributed_device, broadcast_object
from open_clip_train.precision import get_autocast
import random
from open_clip_train.prompt_templates import template_dict
from open_clip_train.benchmark_dataset_info import BENCHMARK_DATASET_INFOMATION
...
"""
Adapted from SkyScript (https://github.com/wangzhecheng/SkyScript)
"""
...
class CsvDatasetForClassification(Dataset):
    ...
    def __init__(self, input_filename, transforms, img_key, label_key, classnames, sep="\t", debugging=False,
                 root_data_dir=None):
...
def test_zero_shot_classification(model, dataloader, label_list, is_binary, args, dataset_name='unnamed',
                                  debugging=False):
    # logging.info('Starting zero-shot classification test.')
    model.eval()
    # For classifier, we refer to the open_clip repo rather than to SkyScript.
    context_length = 77 if args.long_clip == 'disable' else 248
    tokenizer = get_tokenizer(args.model, context_length=context_length)
    ...
    for dataset_name in benchmark_dataset_info:
        ...
        if benchmark_dataset_info[dataset_name]['classification_mode'] == 'binary':
            ds = CsvDatasetForClassificationBinary(
                ...
                root_data_dir=args.test_data_dir,
            )
        else:
...
def test(args):
    ...
    random_seed(42, 0)  # Keep consistent with SkyScript
