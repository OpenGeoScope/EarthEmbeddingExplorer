...
class AugmentationCfg:
    ...
    color_jitter: Optional[Union[float, Tuple[float, float, float], Tuple[float, float, float, float]]] = (0.4, 0.4, 0.2, 0.1)  # (0.4, 0.4, 0.2, 0.1) None
    re_prob: Optional[float] = None
    re_count: Optional[int] = None
    use_timm: bool = False

    # params for simclr_jitter_gray
    color_jitter_prob: float = 0.8     # 0.8    None
    gray_scale_prob: float = 0.2       # 0.2   None
...
class DataAugmentationMulticrop(object):
    """
    Refer to DINO augmentation code https://github.com/facebookresearch/dino/blob/main/main_dino.py#L419
    """

    def __init__(self, mean, std, image_size, local_crops_number, local_method):
        self.local_crops_number = local_crops_number
        self.local_crops_scale = (0.3, 0.7)     # Hard coding right now
        self.local_method = local_method
        self.grid = factor_pair(local_crops_number)
    ...
    def __call__(self, image):
        ...
        # ----- Random cropping from global image for local views -----
        if self.local_method == 'randomcrops':
            for idx in range(self.local_crops_number):
                i, j, h, w = RandomResizedCrop.get_params(
                    image, scale=self.local_crops_scale, ratio=(3 / 4, 4 / 3)
                )
                x1 = j / width
                y1 = i / height
                x2 = (j + w) / width
                y2 = (i + h) / height
                bboxes.append(torch.tensor([x1, y1, x2, y2, 1.0]))
                local_imgs.append(self.local_transfo(image.crop((j, i, j + w, i + h))))

        # ----- Grid cropping from global image for local views (as in CLIPSelf https://arxiv.org/abs/2310.01403) -----
        elif self.local_method == "grids":
            M, N = self.grid
            ...
            grid_boxes = torch.cat([torch.cat([x0y0s, x1y1s], dim=-1).view(-1, 4), torch.ones(M * N, 1)],
                                   dim=1)  # [M*N, 5]

            for box in grid_boxes:
                x1, y1, x2, y2, _ = box
                j, i = int(x1 * width), int(y1 * height)
                jw, ih = int((x2 - x1) * width), int((y2 - y1) * height)
                crop = image.crop((j, i, j + jw, i + ih))
                local_imgs.append(self.local_transfo(crop))
                bboxes.append(box)
        else: raise "Unrecognized local_method."

        return {
            "global": self.global_transfo(image),
            "locals": torch.stack(local_imgs),
...
def image_transform(
    ...
    if is_train:
        ...
        if use_timm:
            ...
            train_transform = DataAugmentationMulticrop(mean, std, image_size, max_boxes, local_method)
        else:
            train_transform = [
                # RandomResizedCrop(
                #     image_size,
