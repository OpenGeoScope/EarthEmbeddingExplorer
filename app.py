...
def build_filter_options(enable_time=False, start_date="2016-01-01", end_date="2024-12-31",
                         enable_geo=False, lat_min=-90, lat_max=90, lon_min=-180, lon_max=180):
    """Pack UI filter controls into a single dict for search functions."""
    return {
        "time": {"enabled": enable_time, "start": start_date, "end": end_date},
        "geo": {"enabled": enable_geo, "lat_min": lat_min, "lat_max": lat_max,
...
def search_text(query, threshold, model_name, filter_options=None):
    ...
    try:
        ...
        geo_dist_map, df_filtered = plot_geographic_distribution(df_for_geo, probs_for_geo, threshold/1000.0, title=f'Similarity to "{query}" ({model_name})')

        # Handle 0 results after filtering
        if len(top_indices) == 0:
            status_msg = "No results found with current filter settings.\nTry relaxing the filters or adjusting the threshold."
            yield gr.update(visible=False), [], status_msg, None, [geo_dist_map], df_filtered, gr.update(value=geo_dist_map, visible=True)
            return
        ...
        yield gr.update(visible=False), gallery_items, status_msg, fig_results, [geo_dist_map, fig_results, results_txt, top_results_meta, model_name], df_filtered, gr.update(value=geo_dist_map, visible=True)
    ...
    try:
        ...
        if model_name == "SatCLIP":
            if multiband_data is not None:
                ...
                yield None, None, ("⚠️ SatCLIP requires multi-spectral Sentinel-2 input (12/13 bands).\n\n"
                    ...
                    "or switch to DINOv2 / SigLIP / FarSLIP for RGB image retrieval."), None, None, None, None
                return
        else:
        ...
        # Show geographic distribution (not timed)
        geo_dist_map, df_filtered = plot_geographic_distribution(df_for_geo, probs_for_geo, threshold/1000.0, title=f'Similarity to Input Image ({model_name})')
...
def search_location(lat, lon, threshold, filter_options=None):
    ...
    try:
        ...
        # Show geographic distribution (not timed)
        geo_dist_map, df_filtered = plot_geographic_distribution(df_for_geo, probs_for_geo, threshold/1000.0, title=f'Similarity to Location ({lat}, {lon})')
...
def search_mixed(query_text, query_image, lat, lon, 
                 weight_text, weight_image, weight_location,
                 threshold, model_name, filter_options=None,
                 multiband_data=None):
    """
    ...
    try:
        ...
        if not use_text and not use_image and not use_location:
            yield None, None, "Please provide at least one query (text, image, or location) with weight > 0.", None, None, None, None
            return
        ...
        if need_alignment:
            ...
            if len(common_pids) == 0:
                yield None, None, "No common product IDs between models. Cannot perform mixed search.", None, None, None, None
                return
        ...
        n_samples = len(df_ref) if need_alignment else len(df_ref)
        final_scores = np.zeros(n_samples, dtype=np.float32)
        
        # Normalize weights
        total_weight = 0
        if use_text: total_weight += weight_text
        if use_image: total_weight += weight_image
        if use_location: total_weight += weight_location
        ...
        if use_location:
            status_msg = ""
            if use_text: status_msg += "Encoding text... ✓\n"
            if use_image: status_msg += "Encoding image... ✓\n"
        ...
        if use_text: status_msg += "Encoding text... ✓\n"
        if use_image: status_msg += "Encoding image... ✓\n"
        if use_location: status_msg += "Encoding location... ✓\n"
        ...
        geo_dist_map, df_filtered = plot_geographic_distribution(
            df_for_geo, probs_for_geo, threshold/1000.0, 
            title=f'Mixed Search: {query_info}'
        )
        ...
        # --- Download images ---
        yield gr.update(visible=False), None, status_msg + "Retrieving similar images... ✓\nDownloading images...", None, None, df_filtered, gr.update(value=geo_dist_map, visible=True)
        t0 = time.time()
        top_indices = top_indices[:10]
        ...
        top_results_meta = [{'id': r['id'], 'lat': r['lat'], 'lon': r['lon'], 'score': r['score']} for r in results]
        yield gr.update(visible=False), gallery_items, status_msg, fig_results, [geo_dist_map, fig_results, results_txt, top_results_meta, ref_model_name], df_filtered, gr.update(value=geo_dist_map, visible=True)
...
with gr.Blocks(title="EarthEmbeddingExplorer", css="""
.filter-checkbox {
...
""") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=4):
            with gr.Tabs():
                ...
                with gr.TabItem("Mixed Search") as tab_mixed:
                    ...
                    mixed_text_input = gr.Textbox(
                        ...
                        placeholder="e.g., tropical rainforest, glacier, urban area"
                    )
                    ...
                    with gr.Row():
                        ...
                        weight_location = gr.Slider(minimum=0, maximum=1, value=0.33, step=0.01, label="Location Weight")
            ...
            # Filter controls
            enable_time_filter = gr.Checkbox(label="📅 Enable Time Filter", value=False, elem_classes=["filter-checkbox"])
            with gr.Row():
            ...
            enable_geo_filter = gr.Checkbox(label="🌍 Enable Geo Filter (Bounding Box)", value=False, elem_classes=["filter-checkbox"])
            with gr.Row():
    ...
    plot_map.select(
        fn=handle_map_click,
        inputs=[map_data_state],
        outputs=[mixed_lat, mixed_lon, mixed_pid, mixed_click_status]
    )
    ...
    enable_time_filter.change(
        fn=toggle_time_filter,
        inputs=[enable_time_filter],
        outputs=[time_start, time_end]
    )
    
    enable_geo_filter.change(
        fn=toggle_geo_filter,
        inputs=[enable_geo_filter],
        outputs=[geo_lat_min, geo_lat_max, geo_lon_min, geo_lon_max]
    )

    # Wrapper functions: pack UI filter controls into filter_options dict
    _filter_inputs = [enable_time_filter, time_start, time_end,
                      enable_geo_filter, geo_lat_min, geo_lat_max, geo_lon_min, geo_lon_max]

    def _wrap_search_text(query, threshold, model_name,
                          enable_time, start_date, end_date,
                          enable_geo, lat_min, lat_max, lon_min, lon_max):
        fo = build_filter_options(enable_time, start_date, end_date,
                                  enable_geo, lat_min, lat_max, lon_min, lon_max)
        yield from search_text(query, threshold, model_name, fo)

    def _wrap_search_image(image_input, threshold, model_name,
                           enable_time, start_date, end_date,
                           enable_geo, lat_min, lat_max, lon_min, lon_max,
                           multiband_data=None):
        fo = build_filter_options(enable_time, start_date, end_date,
    ...
    def _wrap_search_location(lat, lon, threshold,
                              enable_time, start_date, end_date,
                              enable_geo, f_lat_min, f_lat_max, f_lon_min, f_lon_max):
        fo = build_filter_options(enable_time, start_date, end_date,
                                  enable_geo, f_lat_min, f_lat_max, f_lon_min, f_lon_max)
        yield from search_location(lat, lon, threshold, fo)

    def _wrap_search_mixed(query_text, query_image, lat, lon,
                           w_text, w_image, w_location,
                           threshold, model_name,
                           enable_time, start_date, end_date,
                           enable_geo, f_lat_min, f_lat_max, f_lon_min, f_lon_max):
        fo = build_filter_options(enable_time, start_date, end_date,
                                  enable_geo, f_lat_min, f_lat_max, f_lon_min, f_lon_max)
        yield from search_mixed(query_text, query_image, lat, lon,
                                w_text, w_image, w_location,
                                threshold, model_name, fo)

    # Search Event (Text)
    search_btn.click(
        fn=_wrap_search_text,
        inputs=[query_input, threshold_slider, model_selector_text] + _filter_inputs,
    ...
    search_mixed_btn.click(
        ...
        inputs=[mixed_text_input, mixed_image_input, mixed_lat, mixed_lon,
                weight_text, weight_image, weight_location,
                threshold_slider, model_selector_mixed] + _filter_inputs,
        outputs=[plot_map_interactive, gallery_images, status_output, results_plot, current_fig, map_data_state, plot_map]
    )
    ...
    save_btn.click(
        fn=save_plot,
        inputs=[current_fig, download_mode],
